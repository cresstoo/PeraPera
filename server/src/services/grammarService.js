...
class GrammarService {
    static analyzeGrammar(words) {
        // 去掉句号
        const contentWords = words.filter(w => w[1] !== '記号');
        // 分析结构
        const structure = {
            hasSubject: contentWords.some((w, i) => w[1] === '名詞' &&
            ...
            hasPredicate: contentWords.some(w => w[1] === '動詞' ||
                w[1] === '助動詞' ||
                w[1] === '形容詞' ||
                w[1] === '形容動詞'),
            hasModifier: contentWords.some(w => w[1] === '形容詞' ||
                w[1] === '副詞')
        };
        // 分析谓语
        const predicate = {
            type: contentWords.some(w => w[1] === '動詞') ? 'verb' :
                contentWords.some(w => w[1] === '形容詞') ? 'adjective' :
                    contentWords.some(w => w[1] === '助動詞') ? 'copula' :
                        null,
            form: {
                tense: contentWords.some(w => { var _a; return (_a = w[5]) === null || _a === void 0 ? void 0 : _a.includes('過去'); }) ? 'past' : 'present',
                polarity: contentWords.some(w => w[0].includes('ない')) ? 'negative' : 'positive',
                style: contentWords.some(w => ['です', 'ます'].includes(w[0])) ? 'polite' : 'plain',
                aspect: null
            }
        };
        return {
    ...
    static calculateScore(words, analysis) {
        // 去掉句号
        const contentWords = words.filter(w => w[1] !== '記号');
        ...
        // 3. 检查基本句型
        const hasSubject = contentWords.some((w, i) => w[1] === '名詞' &&
            i + 1 < contentWords.length &&
            contentWords[i + 1][0] === 'は');
        const hasPredicate = contentWords.some(w => w[1] === '動詞' ||
            w[1] === '助動詞');
        ...
        if (hasParticles)
            details.push('助词使用恰当');
        details.push('基本语序正确');
        return Object.assign(Object.assign({ total }, score), { details });
    }
}
